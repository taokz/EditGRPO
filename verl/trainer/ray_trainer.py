...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0, norm_adv_by_std_in_grpo=True):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=token_level_rewards, eos_mask=response_mask, index=index, norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
        )
...
class RayPPOTrainer:
    ...
    def _maybe_log_val_generations(self, inputs: List[str], outputs: List[str], labels: List[str], scores: List[float]) -> None:
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        for test_data in self.val_dataloader:
            ...
            test_gen_batch.meta_info["original_batch_size"] = len(test_gen_batch)
            ...
            reward_tensor, reward_metrics, _ = self.val_reward_fn(test_batch)
        ...
        self._maybe_log_val_generations(inputs=sample_inputs, outputs=sample_outputs, labels=sample_labels, scores=sample_scores)
    ...
    def fit(self):
        ...
        # Create a separate generator for gt_indices
        gt_generator = torch.Generator()
        gt_generator.manual_seed(42)  # Set seed for gt_indices only
        # for _ in tqdm(range(self.config.trainer.total_episodes), desc="Episode", position=0): # disabled because the output is memory-costly
        for _ in range(self.config.trainer.total_episodes):
            ...
            for batch_dict in self.train_dataloader:
                ...
                """
                Replace generated responses with ground truth responses for some samples (indexed by gt_indices)
                """
                # Before generating responses, identify which samples should use ground truth
                force_gt_num = self.config.algorithm.force_gt_num if hasattr(self.config.algorithm, "force_gt_num") else 0
                # Convert force_gt_num to integer (number of samples) if it's > 0
                num_force_gt = int(force_gt_num) if force_gt_num > 0 else 0
                # Select random indices for ground truth forcing if needed
                gt_indices = None
                if num_force_gt > 0 and "ground_truth" in batch.non_tensor_batch:
                    num_force_gt = min(num_force_gt, len(batch))
                    # torch.manual_seed(42)
                    # torch.cuda.manual_seed_all(42)  # For multi-GPU setups
                    gt_indices = torch.randperm(len(batch), generator=gt_generator)[:num_force_gt].tolist()
                    # Store these indices in meta_info for later use
                    batch.meta_info["gt_indices"] = gt_indices
                    # Store the original batch size for proper index mapping in workers
                    batch.meta_info["original_batch_size"] = len(batch)
                ...
                # If we have ground truth data and indices, properly prepare them for the generation step
                if gt_indices is not None:
                    gen_batch.meta_info["gt_indices"] = batch.meta_info["gt_indices"]
                    gen_batch.meta_info["original_batch_size"] = batch.meta_info["original_batch_size"]
                    # Include ground_truth if available (used to replace generated responses)
                    if "ground_truth" in batch.non_tensor_batch:
                        gen_batch.non_tensor_batch["ground_truth"] = batch.non_tensor_batch["ground_truth"]

                """
                Activate token & label extraction from rollout responses and ground truth
                """
                # if self.config.worker.rollout.extractor_type == "radgraph":
                #     pass
                # elif self.config.worker.rollout.extractor_type == "ratescore":
                #     pass
                if self.config.worker.rollout.extractor:
                    gen_batch.meta_info["extract_flag"] = True
                    # gen_batch.meta_info["extractor_type"] = self.config.worker.rollout.extractor_type
                    gen_batch.meta_info["extractor_server"] = self.config.worker.rollout.extractor_server
                    gen_batch.meta_info["extractor_num_ops_per_para"] = self.config.worker.rollout.extractor_num_ops_per_para
                    gen_batch.meta_info["extractor_num_para"] = self.config.worker.rollout.extractor_num_para
                    gen_batch.meta_info["extractor_type"] = self.config.worker.rollout.extractor_type
                    gen_batch.meta_info["extractor_similarity_threshold"] = self.config.worker.rollout.extractor_similarity_threshold
                    if "ground_truth" in batch.non_tensor_batch:
                        gen_batch.non_tensor_batch["ground_truth"] = batch.non_tensor_batch["ground_truth"]
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):  # wg: worker group
                        # Pad batch to be divisible by world_size for parallel processing
                        # This ensures each worker gets a similar number of samples
                        # padded_gen_batch, pad_size = pad_dataproto_to_divisor(gen_batch, self.actor_rollout_wg.world_size)
                        # padded_gen_batch.meta_info["original_batch_size"] = len(gen_batch)
                        # gen_batch_output = self.actor_rollout_wg.generate_sequences(padded_gen_batch)
                        # gen_batch_output = unpad_dataproto(gen_batch_output, pad_size=pad_size)
                        
                        gen_batch.meta_info["original_batch_size"] = len(gen_batch)
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        reward_tensor, reward_metrics, _ = self.reward_fn(batch)
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        # norm_adv_by_std_in_grpo = self.config.algorithm.norm_adv_by_std_in_grpo  # GRPO adv normalization factor
                        # compute advantages, executed on the driver process
