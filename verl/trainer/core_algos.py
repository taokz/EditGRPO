...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    ...
    Args:
        ...
        loss_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        loss_agg_mode: (str) choices: "token-mean" / # Computes the average loss over all valid tokens in the batch.
                                      "seq-mean-token-sum" / # Sums the losses within each sequence first, then averages these sums across the batch.
                                      "seq-mean-token-mean" / # Averages the losses within each sequence first, then averages these sequence means across the batch.
                                      "seq-mean-token-sum-norm" / # Sums losses within sequences, sums these sequence sums across the batch, and then normalizes by the sequence length (intended to be constant, as per the comment).
    ...
    if loss_agg_mode == "token-mean":
        ...
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1)  # token-sum --> constant factor = 1 instead of MAX-TOKEN; ref: https://github.com/volcengine/verl/issues/742
...
def compute_policy_loss(
    ...
    Args:
        ...
        loss_agg_mode: (str) choices: "token-mean" /
            ...
            "token-mean" is the default behavior
            reference: https://github.com/volcengine/verl/blob/ad4881e16b6ae12572c5ba54fb0945c674db22f4/verl/trainer/ppo/core_algos.py#L364
    ...
    per_token_ppo_loss = torch.max(pg_losses, pg_losses2)
    pg_loss = agg_loss(loss_mat=per_token_ppo_loss,
                    loss_mask=eos_mask,
                    loss_agg_mode=loss_agg_mode) # Pass the chosen mode
