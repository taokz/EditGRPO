...
class FSDPWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        # If we added new special tokens (e.g., <|pad|>) the tokenizer size may
        # exceed the model's embedding matrix.  Resize the embeddings before
        # tying weights to prevent size mismatch errors at training time.
        if len(self.tokenizer) > model.get_input_embeddings().num_embeddings:
            new_size = len(self.tokenizer)
            self.print_rank0(f"Resizing token embeddings from {model.get_input_embeddings().num_embeddings} to {new_size} to accommodate new special tokens.")
