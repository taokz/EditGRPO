...
def radgraph_compute_score(response_str: str, ground_truth: str, reward_server='http://REWARD_SERVER_HOST:5000/predict', type='cot') -> Dict[str, float]:
    """
    Compute RadGraph score by sending request to the RadGraph server.
    ...
    try:
        ...
        print(f"Exception when calling RadGraph server: {str(e)}")
        # return {"overall": 0.0, "format": 0.0, "accuracy": 0.0}
        return {"overall": 0.0, "format": 0.0, "precision": 0.0, "recall": 0.0, "f1": 0.0}
...
def demo_radgraph_computation():
    ...
    test_cases = [
        {
            "response": "\\boxed{The patient has pneumonia in the right lung.}",
    ...
    print("Running RadGraph Demo...\n")
    for i, test_case in enumerate(test_cases, 1):
        response = test_case["response"]
        reference = test_case["reference"]
        score = radgraph_compute_score(response, reference)
        print(f"Test Case {i}:")
