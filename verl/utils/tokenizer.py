...
def get_tokenizer(model_path: str, **kwargs) -> PreTrainedTokenizer:
    ...
    # Ensure PAD token is distinct from EOS token to avoid data-leakage and spurious
    # "<|endoftext|>" strings when padding.  If the tokenizer has no pad token or
    # its pad id currently points to the EOS token, add a dedicated PAD token.
    if tokenizer.pad_token_id is None or tokenizer.pad_token_id == tokenizer.eos_token_id:
        print("[Tokenizer] Adding separate <|pad|> token (distinct from EOS).")
